...
export default function Company_Login() {
  ...
  const handleSignUpChange = (evt) => {
    ...
    if (evt.target.name === "customername") {
      setIsValidSignupInput({
        ...isValidSignupInput,
      ...
      const response = await fetch(
        "http://localhost:5000/velvethomes/seller/createcomp",
        {
          ...
          body: JSON.stringify({
            ...
            customername: compStateSignup.customername,
          }),
        }
      );
      const json = await response.json();
          ...
          {val === "Sign Up" && (
            <form action="" onSubmit={hadleSignupSubmit} className="clf">
              <div
                ...
                style={{ marginTop: "25px" }}
              >
                <label htmlFor="customername" className="input-clf-label">
                ...
                <input
                  ...
                  id="customername"
                  className="login-company-input"
                  value={compStateSignup.customername}
                  name="customername"
                  onChange={handleSignUpChange}
                  style={
                    isValidSignupInput.customername
                      ? { color: "#1A5D1A", borderBottom: "3px solid #1A5D1A" }
                      : { color: "red", borderBottom: "3px solid red" }
                  }
                />
