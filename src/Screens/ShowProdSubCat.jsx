...
export default function ShowProdSubCat() {
  ...
  /*
  Nested Iteration: Use nested loops to iterate through each product (prods[i]) and each filter (fil.key[j]) in the state.

Filter Matching: Check if the filter key exists in the product's features. If it does, check if the corresponding value is included 
in the filter values (fil.values[j]). If both conditions are met, add the product to the arr array.
  */
