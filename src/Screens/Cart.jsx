...
export default function Cart() {
  ...
  const handleChange=(evt,ind)=>{
    ...
    if(q<1){
      ...
      setTotal(total-arr[ind]*products[ind].price+q*products[ind].price)
      arr[ind]=q
      setQty(arr)
    }
  }
    ...
    <div className="CartWrapper">
            ...
            return (
              <div className="CartCard" key={e._id}>
                ...
                <div className="CartCardMain">
                  <div className="CartCardMainTitle" onClick={()=>navigate(`/velvethomes/showproduct/${e._id}`)}>{e.title}</div>
                  <div className="CartCardMainBody">
                    <div className="CartCardMainBodyHead">Price : </div>
                    <div className="CartCardMainBodyValue">Rs. {e.price}/-</div>
                  </div>

                  <div className="CartCardMainBody">
                    <div className="CartCardMainBodyHead">Quantity : </div>
                    <input type="number" value={qty[ind]} onChange={(evt)=>handleChange(evt,ind)} className="CartCardMainBodyInput" />
                  </div>
                  <div className="CartCardMainBody">
                    <div className="CartCardMainBodyBtn" onClick={()=>removeFromCart(e._id)}>Remove</div>
                  </div>
                </div>
              </div>
            );
      ...
      <div className="CartBill">
        ...
        <div className="CartBillDetail">
          ...
          <div className="CartBillDetailsBtn" onClick={applyCode}>Apply</div>
        </div>
        {
